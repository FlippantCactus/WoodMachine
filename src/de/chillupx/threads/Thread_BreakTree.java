...
public class Thread_BreakTree implements Runnable {
	...
	private List<Block> removeBlocks(Machine m, int height) {
		...
		for(Block block : getBlocksAround(m.getCuttingFace(), height)) {		
			if(block.getType() == Material.LOG || block.getType() == Material.LEAVES) {
				...
				//WORLDGUARD
				if(WoodMachine.getConfigManager().getBoolean(ConfigField.WORLDGUARD_REGIONS)) {
					ApplicableRegionSet regions = WGBukkit.getRegionManager(block.getWorld()).getApplicableRegions(block.getLocation());
					LocalPlayer lplay = WGBukkit.getPlugin().wrapOfflinePlayer(m.getOwner());
					if(!regions.testState(lplay, DefaultFlag.BUILD) && !m.getOwner().isOp()) {
